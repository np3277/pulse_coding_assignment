...
class G2Scraper:
    ...
    def __init__(self, company_name: str):
        # Very naive slug generation; in README you can mention this limitation.
        self.company_name = company_name
        self.slug = company_name.lower().replace(" ", "-")

    def _build_url(self, page: int = 1):
        # G2 often uses ?page= param
        return f"{self.BASE_SEARCH_URL.format(slug=self.slug)}?page={page}"

    def _parse_date(self, date_str: str):
        # Adjust format to real one after checking G2; keep simple now.
    ...
    def fetch_reviews(self, start_date, end_date):
        ...
        while True:
            ...
            # Very basic pagination break condition for 2 hours scope
            page += 1
            if page > 3:  # limit to first 3 pages to avoid over-engineering
                break
